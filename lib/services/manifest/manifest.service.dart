...
class ManifestService {
  ...
  final String manifestFileSizeKey = "manifestFileSize";
  ...
  Future download(String url, {DownloadProgress onProgress}) async {
    ...
    if (totalSize < 1) {
      totalSize = prefs.getInt(manifestFileSizeKey) ?? -1;
    }
    if (totalSize < 1) {
      totalSize = 66666666;
    }
    ...
    prefs.setInt(manifestFileSizeKey, loaded);
    await sink.flush();
    await sink.close();
    await this.load();
  }
  ...
  DestinyInventoryItemDefinition getItemDefinition(int hash) {
    Map<String, dynamic> json = getDefinition(
        DefinitionTableNames.destinyInventoryItemDefinition, hash);
    return DestinyInventoryItemDefinition.fromMap(json);
  }

  DestinyStatDefinition getStatDefinition(int hash) {
    Map<String, dynamic> json =
        getDefinition(DefinitionTableNames.destinyStatDefinition, hash);
    return DestinyStatDefinition.fromMap(json);
  }

  DestinyTalentGridDefinition getTalentGridDefinition(int hash) {
    Map<String, dynamic> json =
        getDefinition(DefinitionTableNames.destinyTalentGridDefinition, hash);
    return DestinyTalentGridDefinition.fromMap(json);
  }

  DestinyInventoryBucketDefinition getBucketDefinition(int hash) {
    Map<String, dynamic> json = getDefinition(
        DefinitionTableNames.destinyInventoryBucketDefinition, hash);
    return DestinyInventoryBucketDefinition.fromMap(json);
  }

  DestinyClassDefinition getClassDefinition(int hash) {
    Map<String, dynamic> json = getDefinition(
        DefinitionTableNames.destinyClassDefinition, hash);
    return DestinyClassDefinition.fromMap(json);
  }

  DestinyRaceDefinition getRaceDefinition(int hash) {
    Map<String, dynamic> json = getDefinition(
        DefinitionTableNames.destinyRaceDefinition, hash);
    return DestinyRaceDefinition.fromMap(json);
  }
