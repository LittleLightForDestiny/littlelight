...
class AddCommunityWishlistFormState extends State<AddCommunityWishlistForm> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
    super.build(context);
    final root = context.watch<AddWishlistsProvider>().isRootFolder;
    if (root) {
  ...
  Widget buildFolderHeader(BuildContext context) {
    final folder = context.watch<AddWishlistsProvider>().currentFolder;
    return Container(
        child: Row(
          children: [
            BackButton(
              onPressed: () {
                context.read<AddWishlistsProvider>().goToRoot();
              },
            ),
            Container(
              width: 8,
  ...
  Widget buildWishlists(BuildContext context) {
    final currentFolder = context.watch<AddWishlistsProvider>().currentFolder;
  ...
  Widget buildWishlistFile(BuildContext context, WishlistFile file) {
    final isAdded = context.watch<AddWishlistsProvider>().isAdded(file);
    ...
    return WishlistFileItem(
      ...
      actions: [
        ElevatedButton(
            ...
            onPressed: () async {
              final provider = context.read<AddWishlistsProvider>();
  ...
  Widget buildWishlistFolder(BuildContext context, WishlistFolder folder) {
    return Container(
      ...
      child: Material(
          ...
          child: InkWell(
              onTap: () {
                context.read<AddWishlistsProvider>().goToFolder(folder);
              },
              child: Container(
