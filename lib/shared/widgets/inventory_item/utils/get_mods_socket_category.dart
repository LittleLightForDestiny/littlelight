...
Future<int?> getModsSocketCategory(ManifestService manifest, DestinyInventoryItemDefinition definition) async {
  final hashes = definition.sockets?.socketCategories?.map((e) => e.socketCategoryHash).toList();
  if (hashes == null) return null;
  final hardCodedHash = hashes.firstWhereOrNull((element) => SocketCategoryHashes.mods.contains(element));
  ...
  final def = defsList.firstWhereOrNull((def) => def.categoryStyle == DestinySocketCategoryStyle.Consumable);
  return def?.hash;
}
