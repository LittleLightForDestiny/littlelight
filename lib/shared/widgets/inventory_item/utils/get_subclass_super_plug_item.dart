...
Future<DestinyInventoryItemDefinition?> getSubclassSuperPlugItem(
  ...
  final categoryDef = defsList.firstWhereOrNull(
      (def) => def.categoryStyle == DestinySocketCategoryStyle.Supers);
  if (categoryDef == null) return null;
  final socketCategory = definition.sockets?.socketCategories
      ?.firstWhereOrNull((c) => c.socketCategoryHash == categoryDef.hash);
