...
Future<int?> getEnergySocketCategoryHash(ManifestService manifest, DestinyInventoryItemDefinition definition) async {
  final hashes = definition.sockets?.socketCategories?.map((e) => e.socketCategoryHash).toList();
  if (hashes == null) return null;
  final hardCodedHash = hashes.firstWhereOrNull((e) => e == SocketCategoryHashes.energyCapacity);
  ...
  final def = defsList.firstWhereOrNull((def) => def.categoryStyle == DestinySocketCategoryStyle.EnergyMeter);
  return def?.hash;
}
