...
Future<int?> getPerksSocketCategory(ManifestService manifest, DestinyInventoryItemDefinition definition) async {
  final hashes = definition.sockets?.socketCategories?.map((e) => e.socketCategoryHash).toList();
  if (hashes == null) return null;
  final hardCodedHash = hashes.firstWhereOrNull((element) => SocketCategoryHashes.perks.contains(element));
  ...
  final def = defsList.firstWhereOrNull((def) => def.categoryStyle == DestinySocketCategoryStyle.Reusable);
  return def?.hash;
}
