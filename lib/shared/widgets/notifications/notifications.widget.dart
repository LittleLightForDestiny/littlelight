...
class NotificationsWidget extends StatelessWidget {
  ...
  Widget buildMainContainer(BuildContext context) {
    return AnimatedContainer(
      ...
      child: Row(
        ...
        children: [buildMainMessage(context), buildRefreshButtonContainer(context)].whereType<Widget>().toList(),
      ),
    );
  }

  Widget? buildSubjects(BuildContext context) {
    final transferActions = _state(context).actionsByType<ItemActionNotification>();
        ...
        child: Stack(
          children: [
            buildMainMessageAnimation(
              context,
              Text("Updating".translate(context).toUpperCase()),
              _state(context).actionIs<UpdateAction>(),
            ),
            buildMainMessageAnimation(
              context,
              Text("Syncing loadouts".translate(context).toUpperCase()),
              _state(context).actionIs<SyncLoadoutsAction>(),
            ),
            buildMainMessageAnimation(
              context,
              Text("Transferring".translate(context).toUpperCase()),
              _state(context).actionIs<TransferNotification>(),
            ),
            buildMainMessageAnimation(
              context,
              Text("Applying changes".translate(context).toUpperCase()),
              _state(context).actionIs<ApplyPlugsNotification>(),
            ),
            buildMainMessageAnimation(
              context,
              Text("Equipping loadout".translate(context).toUpperCase()),
              _state(context).actionIs<EquipDestinyLoadoutNotification>(),
            ),
            buildMainMessageAnimation(
              context,
              Text("Update failed".translate(context).toUpperCase()),
  ...
  Widget buildMainMessageAnimation(BuildContext context, Widget widget, bool visible) => ValueAnimationBuilder(
    (controller) => SizeTransition(
      ...
      child: Container(padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 16), child: widget),
    ),
  ...
  Widget buildRefreshButtonContainer(BuildContext context) {
    return ValueAnimationBuilder(
      (controller) => SizeTransition(sizeFactor: controller, axis: Axis.horizontal, child: buildRefreshButton(context)),
  ...
  Widget buildPersistentNotifications(BuildContext context) {
    ...
    return Column(children: notifications.map((n) => buildPersistentNotification(context, n)).toList());
  }
