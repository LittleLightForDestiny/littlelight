...
class StatValues {
  ...
  int get maximumValue {
    final maximum = scale?.maximumValue ?? 100;
    return _interpolate(maximum);
  }
  ...
  int _interpolate(int value) {
    ...
    final floored = displayValue.floor();
    final diff = displayValue - floored;
    if (diff != .5) {
      return displayValue.round();
    }
    final isEven = floored % 2 == 0;
    if (isEven) return floored;
    return floored + 1;
  }
