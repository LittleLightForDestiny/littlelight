...
class SeasonSlotFilter extends BaseItemFilter<SeasonSlotFilterOptions>
    with ManifestConsumer {
  SeasonSlotFilter(Set<int> values)
      : super(SeasonSlotFilterOptions(values.toSet(), values));
  ...
  Future<bool> filterItem(DestinyItemInfo item) async {
    ...
    if (data.value.contains(-1)) {
      ...
      for (var socket in entries) {
        contains =
            contains || data.value.contains(socket.singleInitialItemHash);
      }
