...
class SettingsView extends StatelessWidget {
          ...
          SingleChildScrollView(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                SettingsOptionWidget("Scroll areas".translate(context).toUpperCase(), buildScrollAreaOptions(context)),
                SettingsOptionWidget(
                ...
                SettingsOptionWidget("Order items by".translate(context).toUpperCase(), buildItemOrderList(context)),
                SettingsOptionWidget(
                  "Order pursuits by".translate(context).toUpperCase(),
                  buildPursuitOrderList(context),
                ),
                SettingsOptionWidget("Priority Tags".translate(context).toUpperCase(), buildPriorityTags(context)),
              ],
            ),
          ),
          Positioned.fill(
            child: AnimatedOpacity(
              ...
              child: DividerIndicatorOverlay(threshold: _state.scrollAreaDividerThreshold, enabled: true),
            ),
          ),
        ],
      ),
  ...
  Widget buildWishlistsList(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: wishlists
            .map((w) => WishlistFileItem(file: w, onRemove: () => _bloc.removeWishlist(w), isAdded: true))
            .toList(),
      ),
    );
  }
    ...
    return Container(
      child: IntrinsicHeight(
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: <Widget>[
            buildCharacterOrderItem(context, "Last played".translate(context), CharacterSortParameterType.LastPlayed),
            Container(width: 4),
            ...
            Container(width: 4),
            buildCharacterOrderItem(context, "Last created".translate(context), CharacterSortParameterType.LastCreated),
          ],
        ),
      ),
    );
      ...
      child: Material(
        ...
        child: InkWell(
          child: Container(
            ...
            child: Text(label, style: context.textTheme.button, textAlign: TextAlign.center),
          ),
          onTap: () {
  ...
  Widget buildItemOrderList(BuildContext context) {
    ...
    return ReorderableList(
      ...
      onReorderItem: (o, n) => _bloc.reorderItemOrdering(o, n),
    );
  }
  ...
  Widget buildPursuitOrderList(BuildContext context) {
    ...
    return ReorderableList(
      ...
      onReorderItem: (o, n) => _bloc.reorderPursuitOrdering(o, n),
    );
  }
    ...
    return Column(
      children: [
        ...
        buildScreenAreaSelector(
          context,
          Text("Top half".translate(context).toUpperCase(), style: context.textTheme.highlight),
          _state.topScrollArea,
          (value) => _bloc.topScrollArea = value,
        ),
        buildScreenAreaSelector(
          context,
          Text("Bottom half".translate(context).toUpperCase(), style: context.textTheme.highlight),
  ...
  Widget buildScreenAreaDividerPosition(BuildContext context) {
        ...
        children: [
          Container(
            ...
            child: Row(
              children: [
                Expanded(
                  child: Text("Divider position".translate(context).toUpperCase(), style: context.textTheme.highlight),
                ),
                Text("${_state.scrollAreaDividerThreshold}%", style: context.textTheme.highlight),
              ],
            ),
          ),
          Slider(
  ...
  Widget buildScreenAreaHintToggle(BuildContext context) {
    if (_state.topScrollArea == _state.bottomScrollArea) return Container();
    return Container(
      margin: EdgeInsets.only(top: 4),
      decoration: BoxDecoration(color: context.theme.surfaceLayers.layer2, borderRadius: BorderRadius.circular(4)),
