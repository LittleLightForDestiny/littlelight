...
import 'package:little_light/modules/initial/blocs/select_membership.bloc.dart';
...
class SelectMembershipSubPageState extends SubpageBaseState<SelectMembershipSubPage> with AuthConsumer {
  ...
  void loadAccounts() async {
    context.read<SelectMembershipBloc>().loadAccounts();
  }
  ...
  Widget buildCurrentAccount(BuildContext context) {
    final mainAccount = context.watch<SelectMembershipBloc>().currentAccount;
    if (mainAccount != null) {
  ...
  Widget buildOtherAccounts(BuildContext context) {
    final accounts = context.watch<SelectMembershipBloc>().otherAccounts;
    if (accounts != null) {
      return Column(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (account.bungieNetUser?.membershipId != null)
                      Text(
                        "membershipID: {membershipID}".translate(
                          context,
                          replace: {
  ...
  Widget buildMembership(
    ...
    GroupUserInfoCard destinyInfoCard,
            ...
            child: Row(
              children: [
                ...
                if ((destinyInfoCard.applicableMembershipTypes?.length ?? 0) > 1)
                  Row(
                    children:
                        destinyInfoCard.applicableMembershipTypes
                            ?.map(
                              (m) => Container(
                                decoration: BoxDecoration(color: m.color, borderRadius: BorderRadius.circular(16)),
