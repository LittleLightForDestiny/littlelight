...
class LandscapeItemCoverWidget extends StatelessWidget {
  ...
  String get tag {
    List<dynamic> params = [
      ...
      item?.item?.itemHash ?? definition?.hash,
      item?.ownerId
    ];
...
class LandscapeItemCoverDelegate extends SliverPersistentHeaderDelegate
  ...
  Widget build(
    ...
    double expandRatio =
        min(1, max(0, 1 - (shrinkOffset) / (maxHeight - minHeight)));
  ...
  double bottomPadding(BuildContext context, double expandRatio) => max(
      convertSize(48.0, context), MediaQuery.of(context).viewPadding.bottom);
  ...
  Widget rightColumn(BuildContext context, double expandRatio) {
    final height = min(MediaQuery.of(context).size.height,
  ...
  Widget leftColumn(BuildContext context, double expandRatio) {
    ...
    var exoticIntrinsicPerkCategory = definition?.sockets?.socketCategories
        ?.firstWhere(
            ...
            orElse: () => null);
    final topPadding =
        (this.topPadding(context, expandRatio) + convertSize(180.0, context)) *
                expandRatio +
    ...
    return Positioned(
        ...
        child: Opacity(
            ...
            child: SingleChildScrollView(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: <Widget>[
                if (definition?.displayProperties?.description?.isNotEmpty ??
                    false)
                  Container(
  ...
  Widget buildPowerCap(BuildContext context) {
    var versionNumber =
        item?.item?.versionNumber ?? definition?.quality?.currentVersion;
  ...
  Widget buildBackgroundImage(BuildContext context) {
    ...
    if (item?.item?.overrideStyleItemHash != null) {
      return DefinitionProviderWidget<DestinyInventoryItemDefinition>(
          item?.item?.overrideStyleItemHash, (def) {
