...
class SupporterCharacterWidget extends StatefulWidget {
  ...
  final String? link;
  final Widget? badge;
  const SupporterCharacterWidget(this.membershipId, this.membershipType,
...
class SupporterCharacterWidgetState extends State<SupporterCharacterWidget>
  ...
  loadCharacters() async {
    ...
    list.sort((charA, charB) {
      DateTime dateA = DateTime.tryParse(charA.dateLastPlayed ?? "") ??
          DateTime.fromMillisecondsSinceEpoch(0);
      DateTime dateB = DateTime.tryParse(charB.dateLastPlayed ?? "") ??
          DateTime.fromMillisecondsSinceEpoch(0);
  ...
  Widget buildPlayerClass(BuildContext context) {
    ...
    return Row(children: [
      classHash != null
          ? ManifestText<DestinyClassDefinition>(classHash,
              textExtractor: (def) {
              return def.genderedClassNamesByGenderHash?["$genderHash"] ??
      ...
      raceHash != null
          ? ManifestText<DestinyRaceDefinition>(raceHash, textExtractor: (def) {
              return def.genderedRaceNamesByGenderHash?["$genderHash"] ??
                  def.displayProperties?.name;
            }, style: const TextStyle(fontSize: 12))
          : Container(),
    ]);
