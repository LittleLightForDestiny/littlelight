  ...
  String getApplyModsErrorMessage(BungieApiException? exception) {
    switch (exception?.errorCode) {
      case PlatformErrorCodes.DestinySocketActionNotAllowed:
        return "The selected change can't be applied throught the API or third party apps like Little Light.".translate(
          this,
          useReadContext: true,
        );
      ...
      default:
        if (exception != null) {
          getInjectedAnalyticsService().registerNonFatal(
            Exception("Got an unexpected error code while applying plugs ${exception.errorCode}"),
            null,
          );
        }
